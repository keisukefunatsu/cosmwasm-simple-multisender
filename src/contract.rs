...
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, RecipentInfo};
use crate::state::{MinterData, TokenInfo, BALANCES, LOGO, MARKETING_INFO, TOKEN_INFO};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:cosmwasm-simple-multi-sender";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::MultiTransfer { recipients } => {
            let sender_balance = BALANCES
                .may_load(deps.storage, &info.sender)?
                .unwrap_or_default();
            execute_multi_transfer(deps, env, info, sender_balance, recipients)
        }
...
pub fn execute_multi_transfer(
    ...
    _env: Env,
    info: MessageInfo,
    sender_balance: Uint128,
    recipients: Vec<RecipentInfo>,
) -> Result<Response, ContractError> {
    let mut sum: Uint128 = Uint128::from(0u128);
    for recipient in &recipients {
        if recipient.amount == Uint128::zero() {
            return Err(ContractError::InvalidZeroAmount {});
        }
    ...
    if sum > sender_balance {
        return Err(ContractError::MultiTransferOverFlow {});
    }

    for recipient in recipients {
        let rcpt_addr = deps.api.addr_validate(&recipient.address)?;
        BALANCES.update(
            deps.storage,
            &info.sender,
            |balance: Option<Uint128>| -> StdResult<_> {
                Ok(balance.unwrap_or_default().checked_sub(recipient.amount)?)
            },
        )?;
        BALANCES.update(
            deps.storage,
            &rcpt_addr,
            |balance: Option<Uint128>| -> StdResult<_> {
                Ok(balance.unwrap_or_default() + recipient.amount)
            },
        )?;
    }
    let res = Response::new()
...
mod tests {
    ...
    fn multi_transfer() {
        let mut deps = mock_dependencies_with_balance(&coins(2, "token"));
        let initial_amount = Uint128::from(32222222u128);
        let owner_address = String::from("owner");
        let empty_amount_address = String::from("empty_amount");

        let recipient1 = RecipentInfo {
            address: String::from("addr0001"),
            amount: Uint128::from(11111111u128),
        };
        let recipient2 = RecipentInfo {
            address: String::from("addr0002"),
            amount: Uint128::from(11111111u128),
        };
        let recipient3 = RecipentInfo {
            address: String::from("addr0003"),
            amount: Uint128::from(11111111u128),
        };

        let zero_amount_recipient = RecipentInfo {
            address: String::from("zeroAmount"),
            amount: Uint128::from(0u128),
        };
        let invalid_address_recipient = RecipentInfo {
            address: String::from(""),
            amount: Uint128::from(10u128),
        };

        let available_amount_recipients = vec![recipient1.clone(), recipient2.clone()];
        let zero_amount_recipients = vec![
            zero_amount_recipient,
            recipient1.clone(),
            recipient2.clone(),
        ];
        let invalid_address_recipients = vec![
            recipient1.clone(),
            recipient2.clone(),
            invalid_address_recipient,
        ];
        let too_much_amount_recipients = vec![recipient1.clone(), recipient2.clone(), recipient3];

        do_instantiate(deps.as_mut(), &owner_address, initial_amount);

        // cannot transfer if zero amount included
        let info = mock_info(&owner_address.as_ref(), &[]);
        let env = mock_env();
        let msg = ExecuteMsg::MultiTransfer {
            recipients: zero_amount_recipients,
        };
        let err = execute(deps.as_mut(), env, info, msg).unwrap_err();
        assert_eq!(err, ContractError::InvalidZeroAmount {});

        // cannot transfer if invalid address included
        let info = mock_info(owner_address.as_ref(), &[]);
        let env = mock_env();
        let msg = ExecuteMsg::MultiTransfer {
            recipients: invalid_address_recipients,
        };
        let err = execute(deps.as_mut(), env, info, msg).unwrap_err();
        assert!(
            matches!(err, ContractError::Std(StdError::GenericErr { msg }) if msg == "Invalid input: human address too short")
        );
        ...
        let env = mock_env();
        let msg = ExecuteMsg::MultiTransfer {
            recipients: too_much_amount_recipients,
        };
        let err = execute(deps.as_mut(), env, info, msg).unwrap_err();
        assert!(matches!(err, ContractError::MultiTransferOverFlow {}));

        // cannot send from empty account
        let info = mock_info(empty_amount_address.as_ref(), &[]);
        let env = mock_env();
        let msg = ExecuteMsg::MultiTransfer {
            recipients: available_amount_recipients.clone(),
        };
        let err = execute(deps.as_mut(), env, info, msg).unwrap_err();
        assert!(matches!(err, ContractError::MultiTransferOverFlow {}));
        ...
        let transfer_sum: Uint128 = available_amount_recipients.iter().map(|r| r.amount).sum();
        println!("transfer_sum: {:?}", transfer_sum);
        let remainder = initial_amount.checked_sub(transfer_sum).unwrap();
        println!("remainder: {:?}", remainder);
        assert_eq!(get_balance(deps.as_ref(), &owner_address), remainder);
        assert_eq!(
            get_balance(deps.as_ref(), recipient1.address.clone()),
            recipient1.amount
        );
        assert_eq!(
            query_token_info(deps.as_ref()).unwrap().total_supply,
